...
The system described in this document is a consolidation of the voter registers of the municipalities. It is synchronized each night and serves as the base for the processes of votations and elections. The software presented here is a voting register with the product name `VOTING Stimmregister` of the company Abraxas Informatik AG (`Abraxas`).
...
- We recognize reports from experts as an important contribution to the improvement of the security of the system `VOTING Stimmregister`.
...
In scope for the Bug Bounty Program is a dedicated non-productive instance of the running product `VOTING Stimmregister`, including the necessary infrastructure. The product `VOTING Stimmregister` and its infrastructure include for example:
...
Please note that in a normal, productive setting `VOTING Stimmregister` is not exposed to the Internet.
...
- Issue / security advisory to the Abraxas Product Security Incident Response Team: psirt-voting@abraxas.ch
    ...
    Please ensure to report only findings having an impact on security of the product `VOTING Stimmregister` (see chapter "Scope"). Your report must contain a clear proof of concept that demonstrates the scenario. Findings which have no impact on security or which are purely theoretical are not eligible.

Your submission must contain the following details:

- Attack scenario including impact on the product `VOTING Stimmregister` in form of a step-by-step procedure to reproduce the issue
...
Abraxas will communicate transparently about results and findings as soon as the public release starts on GitHub under [voting-Stimmregister-docs / Security Advisory](https://github.com/abraxas-labs/voting-stimmregister-docs/security/advisories).
